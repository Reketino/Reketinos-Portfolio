...
export async function GET(request) {
  ...
  if (!month || !day) {
    return new Response(JSON.stringify({ error: "Month or day is Lost" }), {
      status: 400,
    });
  }

  const cacheKey = `${month.toString().padStart(2, "0")}-${day
    .toString()
    .padStart(2, "0")}`;

  if (cache.has(cacheKey)) {
    return new Response(JSON.stringify({ fact: cache.get(cacheKey) }), {
      status: 200,
    });
  }
